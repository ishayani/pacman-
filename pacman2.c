...
void followpacman(struct figure *f){ // fllows paman
...
int checkIfGameOver(int input) {
    ...
    if ( (ghost1.x == player.x) && (ghost1.y == player.y) ) {
        return 7;
    }
...
int main(void) {
    ...
    init_pair(6, COLOR_GREEN, COLOR_BLACK);       // Ghost1
    ...
    if (gameState == 1) {
        FILE *score;
        score=fopen("Scores - YOU WON.txt","w");
        ...
        FILE *score;
        score=fopen("Scores NO PONTS.txt","w");
        fprintf(score, "%d",points);
        ...
        FILE *score;
        score=fopen("Scores LOST GAME.txt","w");
